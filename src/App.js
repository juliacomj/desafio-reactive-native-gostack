...
export default function App() {
  ...
  async function handleLikeRepository(id) {
    const repo = projects.find(project => project.id ===id );
  ...
  return (
    <>
      ...
      <SafeAreaView style={styles.container}>
        {projects.map(project => <View key={project.id} style={styles.repositoryContainer} >
        <Text style={styles.repository}>{project.title}</Text>
          <View style={styles.techsContainer}>
            {project.techs.map(tech => <Text key={2*project.id} style={styles.tech}>
              {tech}
            </Text>)}
          </View>
